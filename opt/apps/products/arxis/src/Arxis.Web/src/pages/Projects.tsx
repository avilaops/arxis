        ...
        return (
          <Stack direction="row" spacing={0.5} sx={{ overflow: 'hidden' }}>
            {displayTags.map((tag: string) => (
              <Chip key={tag} label={tag} size="small" icon={<Icons.Tag fontSize="small" />} />
            ))}
            {extraCount > 0 ? (
  ...
  if (!loading && projects.length === 0) {
    return (
      <Box>
        ...
        >
          <Icons.FolderOpen sx={{ fontSize: 120, color: 'text.disabled', mb: 3 }} />
          <Typography variant="h5" gutterBottom>
            Nenhum projeto encontrado
          </Typography>
          <Typography variant="body1" color="text.secondary" sx={{ mb: 4, maxWidth: 500 }}>
            Comece criando seu primeiro projeto do zero, importando dados de uma planilha ou duplicando um projeto existente de outro workspace.
          </Typography>
          ...
          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
            <Button
              ...
              onClick={() => {
                ...
                console.log('Criar projeto do zero');
              }}
            >
              Criar do zero
            </Button>
  ...
  return (
    <Box>
      <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" alignItems={{ xs: 'flex-start', md: 'center' }} spacing={2} sx={{ mb: 3 }}>
        ...
        <Stack direction="row" spacing={1}>
          <Button variant="outlined" startIcon={<Icons.Filter />} onClick={clearFilters}>
            Limpar filtros
          </Button>
          <Button
            variant="contained"
          ...
          <Menu anchorEl={actionsAnchor} open={Boolean(actionsAnchor)} onClose={() => setActionsAnchor(null)}>
            <MenuItem onClick={() => setActionsAnchor(null)}>
              <Icons.Add fontSize="small" sx={{ mr: 1 }} /> Criar do zero
            </MenuItem>
            <MenuItem onClick={() => setActionsAnchor(null)}>
              <Icons.Copy fontSize="small" sx={{ mr: 1 }} /> Duplicar projeto existente
            </MenuItem>
            <Divider />
            <MenuItem onClick={() => setActionsAnchor(null)}>
              <Icons.Upload fontSize="small" sx={{ mr: 1 }} /> Importar (Excel/CSV)
            </MenuItem>
            <MenuItem onClick={() => setActionsAnchor(null)}>
              <Icons.Download fontSize="small" sx={{ mr: 1 }} /> Exportar portfólio
            </MenuItem>
          </Menu>
        </Stack>
      </Stack>
