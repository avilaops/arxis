...
fn test_scientific_types() {
    use avila_dataframe::core::dtype::{Quaternion, GeodesicCoord};
    ...
    assert!((mag_sq - 1.0).abs() < 1e-10, "Quaternion should be normalized");
...
fn test_astronomy_functions() {
    use avila_dataframe::scientific::astronomy::{
        luminosity_distance, angular_separation, absolute_magnitude,
    };
    
    // Test luminosity distance (Hubble law approximation)
    let z = 0.1; // redshift
    let d = luminosity_distance(z);
    assert!(d > 400.0 && d < 450.0, "Distance should be ~427 Mpc for z=0.1");
    
    // Test angular separation
    let sep = angular_separation(0.0, 0.0, 1.0, 1.0);
    assert!(sep > 1.4 && sep < 1.5, "Separation should be ~1.414 degrees");
    
    // Test absolute magnitude
    let m_abs = absolute_magnitude(-26.0, 100.0); // apparent mag, distance in Mpc
    assert!(m_abs < -25.0, "Absolute magnitude should be brighter for nearby objects");
}
...
fn test_ai_standardization() {
    ...
    let standardized = df.standardize(&["feature1", "feature2"]).expect("Standardization failed");
    
    // Check that standardized features have mean ≈ 0 and std ≈ 1
    let feature1 = standardized.column("feature1_std").expect("Column not found");
