...
fn main() {
    ...
    println!("    • Tempo de Hubble: {:.2} Gyr", params.hubble_time() / (365.25 * 24.0 * 3600.0 * 1e9));
    println!("    • Distância de Hubble: {:.2} Gpc\n", params.hubble_distance() / 3.086e25);
    ...
    println!("    • d_L = (1+z) × d_c: {:.3} = {:.3} × {:.3} ✓", 
        dl/3.086e25, 1.0+z_test, dc/3.086e25);
    println!("    • d_A = d_c / (1+z): {:.3} = {:.3} / {:.3} ✓",
        da/3.086e25, dc/3.086e25, 1.0+z_test);
    println!("    • d_L = (1+z)² × d_A: {:.3} = {:.3}² × {:.3} ✓\n",
        dl/3.086e25, 1.0+z_test, da/3.086e25);
    ...
    let z_eq = CosmologicalObservables::matter_radiation_equality(
        params.omega_matter, params.omega_radiation);
    ...
    println!("  ✓ Parâmetros: H₀ = {:.1} km/s/Mpc, Ω_m = {:.3}, Ω_Λ = {:.3}",
    ...
    println!("  ✓ CMB: T₀ = {:.3} K, z_rec ≈ 1100", params.cmb_temperature);
